...
SEOom Builder는 매일 자동으로 각 사이트별로 데이터베이스와 파일 업로드를 백업합니다.

### 자동 백업 스케줄
- **백업 생성**: 매일 0시 (자정) - 각 사이트별로 개별 백업 생성 (자동 백업 활성화 시에만 실행)
- **백업 보관 기간**: 7일 (1주일 이상 된 백업은 자동 삭제)
- **백업 정리**: 매일 0시 30분
- **자동 백업 ON/OFF**: 백업 관리 페이지에서 토글로 제어 가능

### 백업 방식
- **사이트별 백업**: 각 사이트마다 개별 백업 파일이 생성됩니다
  - 파일명 형식: `backup_site_{사이트슬러그}_{날짜_시간}.sql`
  - 각 사이트의 데이터와 파일이 별도로 백업됩니다
...
- **파일명 형식**: 
  - 전체 백업: 
    - 데이터베이스: `backup_{데이터베이스명}_{날짜_시간}.sql`
    - 파일: `backup_{데이터베이스명}_{날짜_시간}_files.tar.gz`
  - 사이트 백업: 
    - 데이터베이스: `backup_site_{사이트슬러그}_{날짜_시간}.sql`
    - 파일: `backup_site_{사이트슬러그}_{날짜_시간}_files.tar.gz`
...
백업 관리 페이지(`/master/backup`)에서 자동 백업을 활성화하거나 비활성화할 수 있습니다:
- **활성화**: 매일 0시에 자동으로 백업이 생성됩니다.
- **비활성화**: 자동 백업이 중지되며, 수동 백업은 계속 사용할 수 있습니다.
...
4. 파일 업로드(storage/app/public)도 자동으로 백업됩니다 (tar.gz 형식).
5. 파일 백업 복원 시 tar 명령어를 사용하세요:
   ```bash
   tar -xzf backup_파일명_files.tar.gz -C storage/app/public/
   ```
