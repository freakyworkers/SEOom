...
class PaymentController extends Controller
{
     ...
     */
    public function processPlugin(Request $request, $pluginId)
    {
        // Get master site
        $masterSite = Site::getMasterSite();
        if (!$masterSite || !$masterSite->isMasterSite()) {
        ...
        if (!$plugin->is_active) {
            return redirect()->route('store.plugins', ['site' => $masterSite->slug])
                ->with('error', '판매 중인 플러그인이 아닙니다.');
        }

        $targetSiteId = $request->input('target_site_id');
        if (!$targetSiteId) {
            return redirect()->route('store.plugins', ['site' => $masterSite->slug])
                ->with('error', '적용할 사이트를 선택해주세요.');
        }

        $targetSite = Site::findOrFail($targetSiteId);
        if ($targetSite->created_by !== $user->id) {
            abort(403, '이 사이트를 변경할 권한이 없습니다.');
        }

        // 이미 구매한 플러그인인지 확인
        $existingPurchase = PluginPurchase::where('plugin_id', $plugin->id)
        ...
        if ($existingPurchase) {
            return redirect()->route('store.plugins', ['site' => $masterSite->slug])
                ->with('error', '이미 구매한 플러그인입니다.');
        }

        // 가격 계산
        $amount = 0;
        if ($plugin->billing_type === 'one_time') {
            $amount = $plugin->one_time_price;
        } elseif ($plugin->billing_type === 'monthly') {
            $amount = $plugin->price;
        }

        // 무료 플러그인인 경우 바로 구매 완료
        if ($amount == 0 || $plugin->billing_type === 'free') {
            ...
            return redirect()->route('store.plugins', ['site' => $masterSite->slug])
                ->with('success', '플러그인이 구매되었습니다.');
        }

        // 결제 기록 생성
        $orderId = 'plugin_' . $plugin->id . '_' . $targetSite->id . '_' . Str::random(10);
        ...
        // 세션에 플러그인 구매 정보 저장
        session([
            'plugin_purchase_site_id' => $targetSite->id,
            'plugin_purchase_plugin_id' => $plugin->id,
            'plugin_purchase_payment_id' => $payment->id,
        ]);

        // 결제 페이지로 리다이렉트
        return redirect()->route('payment.plugin-checkout', [
    ...
    /**
     * Show plugin checkout page.
     */
    public function pluginCheckout(Request $request)
    {
        // Get master site
        $masterSite = Site::getMasterSite();
        if (!$masterSite || !$masterSite->isMasterSite()) {
        ...
        if (!$orderId || !$pluginId) {
            return redirect()->route('store.plugins', ['site' => $masterSite->slug])
                ->with('error', '주문 정보를 찾을 수 없습니다.');
        }

        $plugin = Plugin::findOrFail($pluginId);
        $payment = Payment::where('toss_order_id', $orderId)->firstOrFail();
        $targetSite = Site::findOrFail(session('plugin_purchase_site_id'));
        ...
        return view('payment.plugin-checkout', compact('masterSite', 'plugin', 'payment', 'targetSite', 'amount', 'orderId'));
    }
}
