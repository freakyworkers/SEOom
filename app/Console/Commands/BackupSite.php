...
class BackupSite extends Command
{
    ...
    public function handle()
    {
        ...
        try {
            ...
            // 파일 업로드 백업 (storage/app/public - 사이트별 디렉토리)
            $this->info("파일 업로드 백업을 시작합니다...");
            $publicPath = storage_path('app/public');
            $sitePublicPath = $publicPath . '/sites/' . $site->slug;
            $filesBackupPath = $backupPath . '/' . str_replace('.sql', '_files.tar.gz', $filename);
            
            // 사이트별 디렉토리가 있으면 백업
            if (file_exists($sitePublicPath) && is_dir($sitePublicPath)) {
                $command = sprintf(
                    'cd %s && tar -czf %s . 2>&1',
                    escapeshellarg($sitePublicPath),
