...
@if(isset($boardPosts) && $boardPosts->count() > 0)
    ...
    <div class="mt-5 mb-4">
        ...
        @if($posts->hasPages())
            @if($board->type === 'pinterest')
                ...
                <div class="mt-4 mb-4 text-center" id="pinterest-detail-load-more-container">
                    <button type="button" 
                            class="btn px-5 py-2" 
                            id="pinterest-detail-load-more-btn"
    ...
    <script>
    (function() {
        const loadMoreBtn = document.getElementById('pinterest-detail-load-more-btn');
        if (!loadMoreBtn) return;
        
        const masonryContainer = document.querySelector('.pinterest-masonry-detail');
        ...
        loadMoreBtn.addEventListener('click', function() {
            ...
            .then(data => {
                if (data.success && data.html) {
                    // 새 게시글 HTML 추가 (클래스명 변경)
                    const modifiedHtml = data.html.replace(/pinterest-masonry-item/g, 'pinterest-masonry-detail-item');
                    masonryContainer.insertAdjacentHTML('beforeend', modifiedHtml);
                    ...
                    if (!data.hasMorePages) {
                        document.getElementById('pinterest-detail-load-more-container').style.display = 'none';
                    }
                }
            })
            .catch(error => {
