...
@section('title', $board->seo_title ?? $board->name)

@push('meta')
@if($board->seo_description)
    <meta name="description" content="{{ $board->seo_description }}">
@endif
@endpush

@push('og-meta')
@if($board->seo_description)
    <meta property="og:description" content="{{ $board->seo_description }}">
@endif
@endpush
        ...
        <div class="mb-3">
            <div class="topic-filter-container d-flex gap-2 align-items-center" style="overflow-x: auto; overflow-y: hidden; -webkit-overflow-scrolling: touch; scrollbar-width: thin;">
                ...
                @foreach($board->topics()->ordered()->get() as $topic)
                    <a href="{{ route('boards.show', ['site' => $site->slug, 'slug' => $board->slug, 'topic' => $topic->id]) }}" 
                       class="btn btn-sm"
...
@if(request('search'))
    ...
    <div class="alert alert-info mb-3">
        ...
        <a href="{{ route('boards.show', array_merge(['site' => $site->slug, 'slug' => $board->slug], request()->except(['search', 'search_type', 'page']))) }}" 
...
@if($posts->count() > 0)
                           ...
                           class="text-decoration-none text-dark">
                            ...
                            @if($board->type === 'event')
                                ...
                                <div class="card-body text-center">
                                    {{-- 주제 배지 (제목 상단) - 이벤트 게시판 사진 타입인 경우 --}}
                                    @if(($board->event_display_type ?? 'photo') === 'photo' && $post->topics->count() > 0)
                                        <div class="mb-2">
    ...
    @elseif($board->type === 'classic')
        ...
        @endphp
        {{-- 데스크톱 테이블 뷰 --}}
        <div class="card bg-white shadow-sm d-none d-md-block {{ $isRoundTheme ? '' : 'rounded-0' }}" style="{{ $isRoundTheme ? 'border-radius: 0.5rem;' : '' }}">
            <div class="table-responsive">
                                    ...
                                    <div class="d-flex align-items-center gap-2">
                                        ...
                                        <a href="{{ route('posts.show', ['site' => $site->slug, 'boardSlug' => $board->slug, 'post' => $post->id]) }}" 
                                           class="text-decoration-none text-dark">
                                            ...
                                            @endif
                                            @if($post->comments->count() > 0)
                                                <span class="fw-bold" style="color: #495057;">+{{ $post->comments->count() }}</span>
                                            @endif
                                            @if($board->enable_likes && $post->like_count > 0)
                                            ...
                                            @auth
                                                @if(auth()->id() !== $post->user_id)
                                                    <div class="dropdown d-inline">
                                                        <a class="text-decoration-none text-muted dropdown-toggle" href="#" role="button" id="boardPostUserDropdown{{ $post->id }}" data-bs-toggle="dropdown" aria-expanded="false">
                                                            <x-user-rank :user="$post->user" :site="$site" />
                                                            {{ $post->user->nickname ?? $post->user->name }}
                                                        </a>
                                                        <ul class="dropdown-menu" aria-labelledby="boardPostUserDropdown{{ $post->id }}">
        ...
        {{-- 모바일 리스트 뷰 --}}
        <div class="card bg-white shadow-sm d-md-none {{ $isRoundTheme ? '' : 'rounded-0' }}" style="{{ $isRoundTheme ? 'border-radius: 0.5rem;' : '' }}">
                            ...
                            <span>
                                ...
                                @else
                                    @auth
                                        @if(auth()->id() !== $post->user_id)
                                            <div class="dropdown d-inline">
                                                <a class="text-decoration-none text-muted dropdown-toggle" href="#" role="button" id="boardPostUserDropdownMobile{{ $post->id }}" data-bs-toggle="dropdown" aria-expanded="false">
                                                    <x-user-rank :user="$post->user" :site="$site" />
                                                    {{ $post->user->nickname ?? $post->user->name }}
                                                </a>
                                                <ul class="dropdown-menu" aria-labelledby="boardPostUserDropdownMobile{{ $post->id }}">
                                                    <li><a class="dropdown-item" href="#" onclick="openSendMessageModal({{ $post->user_id }}, '{{ $post->user->nickname ?? $post->user->name }}'); return false;">쪽지보내기</a></li>
                                                </ul>
                                            </div>
                                        @else
    ...
    @elseif($board->type === 'qa')
        ...
        @php
            ...
            $isRoundTheme = $themeMain === 'round';
        @endphp
        <div class="card bg-white shadow-sm {{ $isRoundTheme ? '' : 'rounded-0' }}" style="{{ $isRoundTheme ? 'border-radius: 0.5rem;' : '' }}">
            <div class="list-group list-group-flush">
            @foreach($posts as $post)
                @php
                    // 게시글 내용에서 이미지가 있는지 확인
...
@else
    <div class="card">
        <div class="card-body text-center py-5">
            ...
            <p class="text-muted mb-4">첫 게시글을 작성해보세요!</p>
            @auth
            ...
            @else
                <a href="{{ route('login', ['site' => $site->slug]) }}" class="btn btn-outline-primary">
                    로그인 후 글쓰기
                </a>
            @endauth
        </div>
