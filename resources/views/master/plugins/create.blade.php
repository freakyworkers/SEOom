...
@section('title', '플러그인 생성')
@section('page-title', '플러그인 생성')
@section('page-subtitle', '새로운 플러그인을 생성합니다')

@section('content')
<div class="card">
    <div class="card-header">
        <h5 class="mb-0"><i class="bi bi-plus-circle me-2"></i>플러그인 정보</h5>
    </div>
    <div class="card-body">
        <form method="POST" action="{{ route('master.plugins.store') }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="row mb-4">
                <div class="col-md-4 mb-3">
                    ...
                    <select class="form-select @error('billing_type') is-invalid @enderror" id="billing_type" name="billing_type" required>
                        ...
                        <option value="monthly" {{ old('billing_type') === 'monthly' ? 'selected' : '' }}>월간 구독</option>
                    </select>
                    @error('billing_type')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                ...
                <div class="col-md-4 mb-3" id="price_container" style="display: {{ old('billing_type') === 'monthly' ? 'block' : 'none' }};">
                ...
                <div class="col-md-4 mb-3" id="one_time_price_container" style="display: {{ old('billing_type') === 'one_time' ? 'block' : 'none' }};">
                    ...
                    <input type="number" class="form-control @error('one_time_price') is-invalid @enderror" id="one_time_price" name="one_time_price" value="{{ old('one_time_price', 0) }}" min="0" step="0.01">
