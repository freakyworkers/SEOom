...
@section('title', '마스터 사이트 관리')
@section('page-title', '마스터 사이트 관리')
@section('page-subtitle', '세움 빌더 기반의 마스터 사이트를 관리할 수 있습니다')

@section('content')
<div class="d-flex justify-content-between align-items-center mb-4">
    <div></div>
    <a href="{{ route('master.master-sites.create') }}" class="btn btn-primary">
        <i class="bi bi-plus-circle me-1"></i>마스터 사이트 생성
    </a>
</div>
...
<div class="card mb-4">
    <div class="card-body">
        <form method="GET" action="{{ route('master.master-sites.index') }}" class="row g-3">
            <div class="col-md-4">
                ...
                <tbody>
                    @foreach($sites as $site)
                        <tr>
                            ...
                            <td>
                                <span class="badge bg-purple">마스터</span>
                            </td>
                            <td>
                            ...
                            <td>
                                <div class="btn-group btn-group-sm">
                                    <a href="{{ route('master.master-sites.show', $site->id) }}" 
                                       class="btn btn-outline-info" title="보기">
                                        <i class="bi bi-eye"></i>
                                    </a>
                                    <a href="{{ route('master.master-sites.edit', $site->id) }}" 
                                       class="btn btn-outline-primary" title="수정">
                                        <i class="bi bi-pencil"></i>
                                    </a>
                                    @if($site->status === 'active')
                                        <form action="{{ route('master.master-sites.suspend', $site->id) }}" 
                                              method="POST" 
                                              class="d-inline">
                                            @csrf
                                    ...
                                    @elseif($site->status === 'suspended')
                                        <form action="{{ route('master.master-sites.activate', $site->id) }}" 
                                              method="POST" 
                                              class="d-inline">
                                            @csrf
...
@else
    <div class="card">
        <div class="card-body text-center py-5">
            ...
            <h4 class="mt-3 mb-2">등록된 마스터 사이트가 없습니다</h4>
            <p class="text-muted mb-4">첫 마스터 사이트를 생성해보세요!</p>
            <a href="{{ route('master.master-sites.create') }}" class="btn btn-primary">
                <i class="bi bi-plus-circle me-1"></i>마스터 사이트 생성
            </a>
        </div>
